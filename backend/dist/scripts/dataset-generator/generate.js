...
async function generateBatch(category, topics, batchNumber) {
    // Folder/file name slug
    const categorySlug = category === "Q&A" ? "qna" : category.toLowerCase();
    const filePath = path.join(process.cwd(), "src", "scripts", "dataset-generator", "output", categorySlug, `${categorySlug}-${batchNumber}.json`);
    if (await fileExists(filePath)) {
        console.log(`⏭️ Skipping ${category} Batch ${batchNumber} (already exists)`);
        return;
    }
...
`,
    ...
    await saveDataset(categorySlug, `${categorySlug}-${batchNumber}`, dataset);
...
async function main() {
    for (const config of generationConfig) {
        for (let i = 0; i < config.batches.length; i++) {
            try {
                await generateBatch(config.category, config.batches[i], i + 1);
            }
            catch (error) {
                console.error(`❌ Failed: ${config.category} Batch ${i + 1}`);
                console.error(error);
            }
        }
    }

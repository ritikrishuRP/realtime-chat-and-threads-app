...
export async function seedThreads() {
    ...
    const outputRoot = path.join(process.cwd(), "src", "scripts", "dataset-generator", "output");
    ...
    for (const folder of folders) {
        ...
        for (const fileName of files) {
            if (!fileName.endsWith(".json"))
                continue;
            const content = await readFile(path.join(folderPath, fileName), "utf8");
    ...
    for (const thread of threads) {
        ...
        const insertedThread = await query(`
  INSERT INTO threads (
    category_id,
    author_user_id,
    title,
  ...
  `, [
        ...
        console.log(`Inserted Thread ID: ${insertedThread.rows[0].id}`);
    }
